...
int main(int argc, char** argv) {
	MPI_Init(&argc, &argv);
	int rank, size, i = 0, relay = 0, index = 0, n = 262144;
	MPI_Comm_size(MPI_COMM_WORLD, &size);
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
	double a[262144] = {[0 ... 262143] = 0.5};
	double start = MPI_Wtime(), startZ, stopZ;
	while (i < 50) {
	if (rank == 0) {
		if (index == 0) {
			startZ = MPI_Wtime();
			MPI_Send(&relay, n, MPI_INT, 1, 0, MPI_COMM_WORLD);
		...
		MPI_Send(&relay, n, MPI_INT, 0, 0, MPI_COMM_WORLD);
